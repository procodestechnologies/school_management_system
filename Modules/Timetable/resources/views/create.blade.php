<x-layouts::app :title="__('Create Timetable Entry')">
    <div class="p-4">

        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="border-b border-gray-200 px-6 py-4 bg-gray-50 rounded-t-lg">
                <h4 class="text-lg font-semibold text-gray-900 mb-0">Create Timetable Entry</h4>
            </div>
            ...
            <form action="{{ route('timetable.store') }}" method="POST">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <flux:select name="day_of_week" label="Day of Week">
                        @foreach ($days as $day)
                            <flux:select.option value="{{ $day }}" :selected="old('day_of_week') === $day">
                                {{ $day }}
                            </flux:select.option>
                        @endforeach
                    </flux:select>
                    ...
                    <flux:input type="text" name="room" label="Room" value="{{ old('room') }}" />
