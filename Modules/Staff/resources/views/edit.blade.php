    ...
    <div class="p-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="border-b border-gray-200 px-6 py-4 bg-gray-50 rounded-t-lg">
                <h4 class="text-lg font-semibold text-gray-900 mb-0">Edit {{ $staff->name }}</h4>
            </div>
            ...
            <form action="{{ route('staff.update', $staff) }}" method="POST">
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <flux:select name="institution_id" label="Institution">
                            ...
                            @foreach ($institutions as $institution)
                                <flux:select.option value="{{ $institution->id }}"
                                    :selected="old('institution_id', $staff->institution_id) == $institution->id">
                                    {{ $institution->name }}
                                </flux:select.option>
                            @endforeach
                        </flux:select>
                        ...
                        <flux:select name="employment_type" label="Employment Type">
                            @foreach (['full_time', 'part_time', 'contract', 'volunteer'] as $type)
                                <flux:select.option value="{{ $type }}"
                                    :selected="old('employment_type', $staff->employment_type) === $type">
                        ...
                        <flux:input type="number" step="0.01" min="0" name="salary"
                            label="Monthly Salary" value="{{ old('salary', $staff->salary) }}" />
                        <flux:select name="status" label="Status">
                            @foreach (['active', 'on_leave', 'suspended', 'resigned', 'terminated'] as $status)
                                <flux:select.option value="{{ $status }}"
                                    :selected="old('status', $staff->status) === $status">
                                    {{ ucfirst(str_replace('_', ' ', $status)) }}
                                </flux:select.option>
                            @endforeach
                        </flux:select>
                        <flux:checkbox name="is_active" value="1" label="Active"
                            :checked="(bool) old('is_active', $staff->is_active)" />
                    </div>

                    <h5 class="text-md font-semibold text-gray-800 mb-1">System Access</h5>
                    <p class="text-sm text-gray-500 mb-3">
                        @if ($staff->user)
                            Signs in as <span class="font-medium">{{ $staff->user->email }}</span>
                            ({{ $staff->user->getRoleNames()->first() ?? 'no role' }}). Leave the password blank to
                            keep the current one.
                        @else
                            This staff member has no login. Tick the box below to give them one.
                        @endif
                    </p>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        @unless ($staff->user)
                        ...
                        <flux:select name="system_role" label="Role">
                            <flux:select.option value="">No system access</flux:select.option>
                            @foreach ($systemRoles as $role)
                                <flux:select.option value="{{ $role }}"
                                    :selected="old('system_role', $staff->user?->getRoleNames()->first()) === $role">
                                    {{ $role }}
                                </flux:select.option>
                            @endforeach
                        </flux:select>
                        <flux:input type="password" name="password"
                            :label="$staff->user ? __('New Password') : __('Password')" />
                    </div>

                    <div class="grid grid-cols-1 gap-4 mb-6">
                        <flux:textarea name="address" rows="2"
                            label="Address">{{ old('address', $staff->address) }}</flux:textarea>
                        <flux:textarea name="notes" rows="2"
                            label="Notes">{{ old('notes', $staff->notes) }}</flux:textarea>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-200 px-6 py-4 bg-gray-50 rounded-b-lg flex justify-end gap-3">
                    ...
                    <flux:button variant="primary" type="submit">Update Staff Member</flux:button>
                </div>
            </form>
        </div>
    </div>
