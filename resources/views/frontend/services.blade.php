<x-layouts::frontend.app title="Services">
    ...
    <section class="py-4 sm:py-8">
        <x-frontend.container>
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                @php
                    $services = [
                        ['icon' => 'academic-cap', 'title' => 'Admissions & Students', 'body' => 'Enroll students, keep guardian and address details in one place, and give every learner a single academic profile.'],
                        ['icon' => 'finger-print', 'title' => 'Biometric Attendance', 'body' => 'Connect ZKTeco devices for tap-in attendance that syncs automatically, with manual registers as a fallback.'],
                        ['icon' => 'banknotes', 'title' => 'Fees & M-Pesa Payments', 'body' => 'Bill fees per term, track balances per student, and accept M-Pesa payments directly into the platform.'],
                        ['icon' => 'clipboard-document-check', 'title' => 'Examinations & Report Cards', 'body' => 'Record marks, compute grades, and generate report cards teachers, parents and admins can all trust.'],
                        ['icon' => 'calendar-days', 'title' => 'Timetable & Classes', 'body' => 'Build class timetables, assign subjects and teachers, and keep every lesson slot conflict-free.'],
                        ['icon' => 'chat-bubble-left-right', 'title' => 'Parent Communication', 'body' => 'A secure self-service chat assistant for results, attendance and fees, verified with a one-time code.'],
                        ['icon' => 'shield-check', 'title' => 'Roles & Permissions', 'body' => 'Fine-grained, role-based access so administrators, teachers and staff only see what they need to.'],
                        ['icon' => 'building-office-2', 'title' => 'Multi-School Support', 'body' => 'Each institution gets its own private, isolated workspace on the same platform.'],
                    ];
                @endphp
    ...
    <section class="py-20 sm:py-28">
        <x-frontend.container class="grid gap-16 lg:grid-cols-2 lg:items-center">
            <div data-animate="left">
                <p class="text-sm font-semibold tracking-wide text-indigo-600 uppercase dark:text-indigo-400">Attendance</p>
                ...
                <ul class="mt-6 space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
                    ...
                    <x-frontend.check-item>Manual entry still available where a device isn't installed</x-frontend.check-item>
                    <x-frontend.check-item>Class-level and school-wide attendance visibility for staff</x-frontend.check-item>
                    <x-frontend.check-item>Parents can check their child's attendance through the chat assistant</x-frontend.check-item>
                </ul>
            </div>

            <div class="relative" data-animate="right">
                <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-zinc-900">
                    ...
                    <ul class="mt-4 space-y-3">
                        @foreach (['Gate reader · 07:42', 'Block A reader · 07:51', 'Block B reader · 08:03'] as $entry)
                            <li class="flex items-center justify-between rounded-lg bg-zinc-50 px-3 py-2.5 text-sm dark:bg-white/5">
        ...
        <x-frontend.container class="grid gap-16 lg:grid-cols-2 lg:items-center">
            <div class="order-2 relative lg:order-1" data-animate="left">
                <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-zinc-900">
                    <div class="flex items-center justify-between border-b border-zinc-100 pb-4 dark:border-white/10">
                        ...
                        <span class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">M-Pesa</span>
                    </div>
                    ...
                    <p class="mt-3 text-xs text-zinc-500 dark:text-zinc-400">80% of this term's fees collected so far</p>
                </div>
            </div>
            ...
            <div class="order-1 lg:order-2" data-animate="right">
                ...
                <ul class="mt-6 space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
                    ...
                    <x-frontend.check-item>Parents check balances any time through the chat assistant</x-frontend.check-item>
                </ul>
            </div>
        </x-frontend.container>
    </section>
    ...
    <section class="py-20 sm:py-28">
        <x-frontend.container class="grid gap-16 lg:grid-cols-2 lg:items-center">
            <div data-animate="left">
                <p class="text-sm font-semibold tracking-wide text-indigo-600 uppercase dark:text-indigo-400">Parent communication</p>
                <h2 class="mt-3 text-3xl font-semibold tracking-tight text-balance text-zinc-900 sm:text-4xl dark:text-white">
            ...
            <div class="relative" data-animate="right">
                <div class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-zinc-900">
                    <div class="flex items-center gap-2 border-b border-zinc-100 pb-3 dark:border-white/10">
                        ...
                        <span class="text-sm font-semibold text-zinc-900 dark:text-white">{{ config('app.name') }} Assistant</span>
                    </div>
    ...
    <section class="pb-20 sm:pb-28">
        <x-frontend.container>
            <x-frontend.cta-band heading="Ready to see the whole platform?" description="Compare plans, or create your school's workspace today.">
